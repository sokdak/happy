...
type WorkflowPublicationRequest = {
  sequence: number
  snapshot: ClaudeWorkflowReducerState
  reset?: WorkflowResetRequest
}

type ActiveWorkflowPublication = {
...
export class ClaudeWorkflowTracker {
  ...
  private activePublication: ActiveWorkflowPublication | null = null
  private queuedReset: WorkflowPublicationRequest | null = null
  private queuedLatest: WorkflowPublicationRequest | null = null
  ...
  handle(message: unknown): void {
    ...
    if (result.publication === 'immediate') {
      this.cancelPending()
      this.enqueueLatest(this.snapshotCurrent())
      return
    }
  ...
  async reset(signal?: AbortSignal): Promise<void> {
    ...
    if (signal?.aborted) {
      throw signal.reason ?? new Error('Claude workflow reset aborted')
    }

    this.cancelPending()
    this.state = {}

    // A reset is a publication barrier. Supersede every older local snapshot
    // immediately so a stalled ACK cannot hold the agent-state lock for the
    // reset's entire timeout budget. ApiSession's optimistic version retry
    // then turns an ACK-lost-but-applied update into version-mismatch -> empty.
    const superseded = new Error('Claude workflow publication superseded by reset')
    if (this.activePublication && !this.activePublication.controller.signal.aborted) {
      this.activePublication.controller.abort(superseded)
    }
    this.queuedLatest = null
    if (this.queuedReset?.reset) {
      this.rejectReset(this.queuedReset.reset, superseded)
    }
    ...
    const reset: WorkflowResetRequest = {
      signal,
      promise,
    ...
    const request: WorkflowPublicationRequest = {
    ...
    if (signal) {
      reset.onAbort = () => this.abortReset(reset, signal.reason)
      signal.addEventListener('abort', reset.onAbort, { once: true })
    }

    this.queuedReset = request
    this.pump()
    return promise
  }
  ...
  dispose(): void {
    ...
    this.seal()
    this.disposed = true
    const reason = new Error('Claude workflow tracker disposed')
    this.cancelQueuedPublications(reason)
    this.abortPublications(reason)
  }
  ...
  async drain(options: { signal?: AbortSignal; timeoutMs?: number } = {}): Promise<void> {
    ...
    const abortPending = () => {
      const reason = drainController.signal.reason
      this.cancelQueuedPublications(reason)
      this.abortPublications(reason)
    }
    ...
    try {
      ...
      while (
        ...
        if (drainController.signal.aborted) break
        this.pump()
        const publication = this.activePublication?.promise
        if (publication) {
  ...
  private snapshotCurrent(): ClaudeWorkflowReducerState {
    ...
    return Object.fromEntries(Object.entries(this.state)) as ClaudeWorkflowReducerState
  }

  private enqueueLatest(snapshot: ClaudeWorkflowReducerState): void {
    const request: WorkflowPublicationRequest = {
      sequence: this.nextSequence++,
      snapshot,
    }
    if (!this.activePublication && !this.queuedReset) {
      this.startPublication(request)
      return
    }
    // A full workflow snapshot supersedes every older unsent normal snapshot.
    // If a reset barrier is waiting or active, keep only the newest state
    // behind it so terminal/empty ordering is preserved without unbounded
    // AsyncLock waiters while the socket is offline.
    this.queuedLatest = request
    this.pump()
  }

  private pump(): void {
    if (this.activePublication || this.disposed) return

    let next: WorkflowPublicationRequest | null = null
    if (this.queuedReset) {
      next = this.queuedReset
      this.queuedReset = null
    } else if (this.queuedLatest) {
      next = this.queuedLatest
      this.queuedLatest = null
    }
    if (!next) return

    if (next.reset?.signal?.aborted) {
      this.rejectReset(
        next.reset,
        next.reset.signal.reason ?? new Error('Claude workflow reset aborted'),
      )
      this.pump()
      return
    }
    this.startPublication(next)
  }

  private startPublication(request: WorkflowPublicationRequest): void {
    const controller = new AbortController()
    let rawPublication: Promise<void>
    try {
      rawPublication = Promise.resolve(this.publish(request.snapshot, controller.signal))
    } catch (error) {
    ...
    const active: ActiveWorkflowPublication = { request, controller, promise: publication }
    this.activePublication = active
    void publication.then(
      () => this.finishPublication(active),
      (error) => this.finishPublication(active, error),
    )
  }

  private finishPublication(active: ActiveWorkflowPublication, error?: unknown): void {
    if (this.activePublication !== active) return
    this.activePublication = null

    if (error === undefined) {
      // Every publication is a complete snapshot, so a newer success
      // supersedes failures from older snapshots.
      this.publicationErrors = []
      if (active.request.reset) this.resolveReset(active.request.reset)
    } else {
      if (!active.controller.signal.aborted) this.publicationErrors.push(error)
      if (active.request.reset) this.rejectReset(active.request.reset, error)
    }

    this.pump()
  }

  private abortReset(reset: WorkflowResetRequest, reason?: unknown): void {
    if (reset.settled) return
    const abortReason = reason ?? new Error('Claude workflow reset aborted')

    if (this.activePublication?.request.reset === reset) {
      if (!this.activePublication.controller.signal.aborted) {
        this.activePublication.controller.abort(abortReason)
      }
      return
    }
    if (this.queuedReset?.reset === reset) {
      this.queuedReset = null
      this.rejectReset(reset, abortReason)
      this.pump()
      return
    }
    this.rejectReset(reset, abortReason)
  ...
  private resolveReset(reset: WorkflowResetRequest): void {
    if (reset.settled) return
    reset.settled = true
    if (reset.signal && reset.onAbort) {
      reset.signal.removeEventListener('abort', reset.onAbort)
    }
    reset.resolve()
  }

  private rejectReset(reset: WorkflowResetRequest, reason?: unknown): void {
    if (reset.settled) return
    reset.settled = true
    if (reset.signal && reset.onAbort) {
      reset.signal.removeEventListener('abort', reset.onAbort)
  ...
  private abortPublications(reason?: unknown): void {
    const controller = this.activePublication?.controller
    if (controller && !controller.signal.aborted) controller.abort(reason)
  }

  private cancelQueuedPublications(reason?: unknown): void {
    if (this.queuedReset?.reset) this.rejectReset(this.queuedReset.reset, reason)
    this.queuedReset = null
    this.queuedLatest = null
  }
