...
describe('ClaudeWorkflowTracker publisher', () => {
  it('publishes starts immediately and coalesces progress updates', async () => {
    vi.useFakeTimers();
    const publish = vi.fn();
  ...
  it('cancels pending progress publication and publishes terminal state immediately', async () => {
    vi.useFakeTimers();
    const publish = vi.fn();
    const tracker = new ClaudeWorkflowTracker(publish);
  ...
  it('keeps only the latest snapshot while an offline publication is stalled', async () => {
    vi.useFakeTimers();
    ...
    const tracker = new ClaudeWorkflowTracker(publish, { coalesceMs: 0 });
  ...
  it('does not let an older reset timeout abort a newer reset or progress publication', async () => {
    ...
    try {
      const olderReset = tracker.reset(olderController.signal);
      const olderResult = olderReset.then(
        () => ({ ok: true as const, error: undefined }),
        (error: unknown) => ({ ok: false as const, error }),
      );
      const newerReset = tracker.reset(newerController.signal);
      tracker.handle(started('workflow-after-reset'));
      ...
      olderController.abort(new Error('older reset timed out'));
